...
const LayoutMe = () => {
  const {head, setHead, setNav, son, setSon, dark, menubar, setMenubar, lang} = useStart()
  ...
  return (
    ...
    <Layout>
      ...
      <Layout className={dark ? "contentMenu site-layout" : 'site-layout'}>
          <HeaderMe />
        <Content
          className={dark ? "contentMenu site-layout-background" : 'site-layout-background'}
          style={{
            margin: "24px 16px",
            padding: 24,
            minHeight: 280,
   ...
   {menubar && <div onClick={spanMenu} className={dark ? "menu menu_dark" : "menu"}>
    <div onClick={() => setMenubar(!menubar)} className="Sabinas_logo zed">
          <img width={50} height={50} src={Logo} alt="logo" />
        ...
        <Menu
          ...
          mode="inline"
          className="menu_Menu"
          onClick={(evt) => {link(evt); setMenubar(!menubar)}}
          defaultSelectedKeys={[son]}
          ...
          items={[
            ...
            {
              key: "6",
              icon:<svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
