...
export const StatePriveder = ({ children }) => {
    ...
    const [soat, setSoat] = useState(`${nol(new Date().getHours())}:${nol(new Date().getMinutes())}:${nol(new Date().getSeconds())}`);

    
    useEffect(() => {
        setInterval(() => {
            setSoat(`${nol(new Date().getHours())}:${nol(new Date().getMinutes())}:${nol(new Date().getSeconds())}`)
        }, 1000);
    }, [soat]);
    ...
    const [vaqt, setVaqt] = useState(`${nol(new Date().getDate())}.${nol(new Date().getMonth()+1)}.${nol(new Date().getFullYear())}`);
    ...
    useEffect(() => {
        setInterval(() => {
         setVaqt(`${nol(new Date().getDate())}.${nol(new Date().getMonth()+1)}.${nol(new Date().getFullYear())}`)
        }, 1000);
     }, [vaqt]);
    ...
    const data = {head, setHead, vaqt,setVaqt, lang, setLang, nav, setNav, son ,setSon, dark, setDark, soat, setSoat, menubar, setMenubar}
